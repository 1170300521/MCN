...
def compact_bilinear_pool(bottom1, bottom2):
    def _generate_sketch_matrix(rand_h, rand_s, output_dim):
        ...
        sparse_sketch_matrix = tf.compat.v1.sparse_reorder(
    ...
    output_shape = bottom1.get_shape().as_list()
    output_shape[0] = -1
    ...
    sketch1 = tf.transpose(tf.compat.v1.sparse_tensor_dense_matmul(sparse_sketch_matrix1,
                                                         bottom1_flat, adjoint_a=True, adjoint_b=True))
    sketch2 = tf.transpose(tf.compat.v1.sparse_tensor_dense_matmul(sparse_sketch_matrix2,
                                                         bottom2_flat, adjoint_a=True, adjoint_b=True))

    # Step 2: FFT
    fft1 = tf.compat.v1.fft(tf.compat.v1.complex(real=sketch1, imag=tf.zeros_like(sketch1)))
    fft2 = tf.compat.v1.fft(tf.compat.v1.complex(real=sketch2, imag=tf.zeros_like(sketch2)))

    # Step 3: Elementwise product
    fft_product = tf.compat.v1.multiply(fft1, fft2)

    # Step 4: Inverse FFT and reshape back
    # Compute output shape dynamically: [batch_size, height, width, output_dim]
    cbp_flat = tf.compat.v1.real(tf.compat.v1.ifft(fft_product))
